...
export default {
  user: {
    ...
    fetchStaffOptions: () =>
      axios.get('/api/users/staff').then(res => res.data.options)
  },
  ...
  ticket: {
    ...
    acceptTicket: ticketId =>
      axios
        .patch(`/api/tickets/accept/${ticketId}`)
        .then(res => res.data.ticket),
    declineTicket: ticketId =>
      axios
        .patch(`/api/tickets/decline/${ticketId}`)
        .then(res => res.data.ticket),
    trackBooking: params =>
      axios
        .get(`/api/tickets/tracker`, { params })
        .then(res => res.data.ticket),
  ...
  payment: {
    fetchPaymentOptions: () =>
      axios.get('/api/payments/payment-options').then(res => res.data.options)
  },
  jobStatus: {
    fetchJobStausOptions: () =>
      axios
        .get('/api/jobstatus/jobstatus-options')
